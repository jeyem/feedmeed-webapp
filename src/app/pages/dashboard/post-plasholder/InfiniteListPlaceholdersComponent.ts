...
export class InfiniteListPlaceholdersComponent implements OnInit, OnDestroy {
  ...
  @ViewChildren(NbListItemComponent, { read: ElementRef })
  ...
  constructor(
    ...
    if (
      ...
      this.window.history.scrollRestoration
    ) {
  ...
  loadNext() {
    ...
    this.newsService
      .load(this.pageToLoadNext, this.pageSize)
      .subscribe(news => {
  ...
  private restoreScrollPosition() {
    concat(
      ...
      .pipe(toArray())
      .subscribe(
        ([oldDimensions, oldScrollPosition, , dimensions, scrollPosition]) => {
